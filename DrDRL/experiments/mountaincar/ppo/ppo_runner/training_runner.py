...
class TrainingRunner(Runner):
    ...
    def run_one_step(self, state, episode_metrics):
        ...
        action = int(self._agent.action(state))  # determine action
        ...
        if self._agent._training:
            ...
            if episode_metrics["t_updates"] % self._agent.n_update == 0:
                ...
                for _ in range(self._agent.PPO_epochs):
                    for states, actions, rewards, dones, next_states in self._agent.memory.get_all_items().batch(
                            batch_size):
                        data = states, actions, rewards, dones, next_states
                        # collect observation to determine major/minor regions (for 'forget mechanism')
                        self._agent._region_locator.add_observations(states)
                        self._agent.update(data)
    ...
    def run_episode(self, episodes, t_updates=0, train=True):
    ...
    def run_training(self, instance_name, save_dir=None):
        ...
        #     self._tracker.add_training_row(row=[instance_name, time.time() - start, episodes, average_reward])
